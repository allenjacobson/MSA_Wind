...
sf_vtrb_cumulative_imgid <- sf_vtrb_cumulative_imgid %>%
  filter(imgid %in% sf_buffered_polygon_subtrip$imgid_chr)

sf_buffered_polygon_subtrip <- sf_buffered_polygon_subtrip %>%
...
#unique_trips <- unique(sf_buffered_polygon_subtrip$tripid_chr) # not all ids in SFCH are in VTRB set
unique_trips <- unique(sf_buffered_polygon_subtrip$imgid)
this_trip <- unique_trips[[163]]

for (this_trip in unique_trips) {
  # filter rows in CH that match trip_id
  # select id only (with geometry)
  this_sfch <- sf_buffered_polygon_subtrip %>%
